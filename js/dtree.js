...
// Node object
function Node(id, pid, name, str, url, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.str = str;
...
// check ：是否显示checkbox
// useSel : 被选择的节点是否高亮显示
// cbCheckFun ：回调，当点击checkbox时执行
// cbClickedCheck ：回调，当点击checkbox后执行
// cbDbClickNode:回调，双击节点时执行
function dTree(objName, check, useSel, cbOnClickCheck, cbClickedCheck,cbDbClickNode) {
	var cb;
	if(!cbOnClickCheck)
		cb='';
	else
	...
	iconPath="images/dtree/";//set the default value, be compatible with dtree
	this.icon = {
			root		: iconPath+'base.gif',
			folder		: iconPath+'folder.gif',
			folderOpen	: iconPath+'folderopen.gif',
			node		: iconPath+'cam.gif',
...
dTree.prototype.add = function(id, pid, name, str, url, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, str, url, title, target, icon, iconOpen, open);
    //return "1;2;3;a;b;C;d;!!!!";
};
...
// added by wangdd 20151216
// 查找是否有节点被选择
dTree.prototype.IsNodeChecked=function()
{
	var len =this.aNodes.length;
	for (var n=0; n<len; n++) 
	{
		var checked = document.getElementById("c"+this.obj+n).checked;
		if(checked)
			return true;
	}
	return false;
}
// added by wangdd 20151215
// 选择特定节点ID的checkbox
dTree.prototype.checkNode=function(nodeId, bCheck)
{
	var len =this.aNodes.length;
	for (var n=0; n<len; n++) 
	{
		if(nodeId==-1) // ID为-1时，处理所有
		{
			document.getElementById("c"+this.obj+n).checked=bCheck;
			this.onclickck(n);
		}
		else
		{
...
// added by wangdd 20151215
// 点击特定checkbox时的回调函数调用
dTree.prototype.onclickck=function(nodeId)
{
	//选择发生改变，执行回调
	//alert("cb111");
	if(this.config.cbOnClickCheck != "")
	{
		//alert("cb");
		var cs = document.getElementById("c"+this.obj+nodeId).checked;
		this.config.cbOnClickCheck(this.aNodes[nodeId], cs);
	}
}
// added by wangdd 20151215
// 点击特定checkbox后的回调函数调用
dTree.prototype.clickedck=function()
{
	//选择发生改变，执行回调
	if(this.config.cbClickedCheck != "")
	{
		this.config.cbClickedCheck();
	}
}
// added by wangdd 20151215
// 双击节点后的回调函数调用
dTree.prototype.dbclicknode=function(nodeId)
{
	//选择发生改变，执行回调
	if(this.config.cbDbClickNode != "")
	{
		this.config.cbDbClickNode(this.aNodes[nodeId]);
	}
}
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		str+= '<input type="checkbox" class="cx" id="c'+  this.obj + nodeId + '" onclick="javascript:'+this.obj+'.cc('+nodeId+');' + this.obj+'.clickedck()' + ';"/>';
	}
	...
	if (node.url != '') {
		...
		// added by wangdd 20151215
		str += ' ondblclick=' + this.obj + '.dbclicknode('+nodeId+');';
