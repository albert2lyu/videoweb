...
$server_mode_str=array(
	"服务器模式(至少选择一种)",
	"Server mode(one or more must be selected)"
);
...
$config_warn_str=array(
	"1、请确认修改的服务器模式和预期设置相同，以免破坏服务器之间业务的协同。\\n2、修改VIS配置将在下一次VIS重启后生效。\\n\\n确认修改VIS配置？",
	"1. First Confirm the change of \'Server Mode\' is same as you expected.\\n2. The change will take effect on the next restart of VIS Server.\\n\\nConfirm to continue?"
);
?>
<?php 
...
if( isset($_POST['vis_config_submit']) )
{
	...
	if( isset($_POST['vci_enable']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set VCIEnable to {$vcienable}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置VCIEnable为{$vcienable}。", CN_LANG);
	}
	...
	if( isset($_POST['vis_mode']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set ServerMode to {$mode}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置ServerMode为{$mode}。", CN_LANG);
	}
	...
	if( isset($_POST['manager_ip']) )
	{
		...
		if( IsIpOk($manager_ip))
		{
			...
			$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set manager ip to {$manager_ip}.");
			$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置管理服务器IP为{$manager_ip}。", CN_LANG);
		}
	}
	...
	if( isset($_POST['return_language']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set ErrCodeLanager to {$return_op_lang}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置ErrCodeLanager为{$return_op_lang}。", CN_LANG);
	}
	...
	if( isset($_POST['transmit_buffer']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set BufferSize to {$buffersize}M.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置BufferSize为{$buffersize}M。", CN_LANG);
	}
	...
	if( isset($_POST['scan_mode']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set ScanSync to {$scan_mode}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置ScanSync为{$scan_mode}。", CN_LANG);
	}
	...
	if( isset($_POST['storage_policy']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set Choice to {$policy}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置Choice为{$policy}。", CN_LANG);
	}
	if( isset($_POST['record_maxlength']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set MaxSize to {$record_length}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置MaxSize为{$record_length}。", CN_LANG);
	}
	if( isset($_POST['record_time']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set MaxTime to {$record_time}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置MaxTime为{$record_time}。", CN_LANG);
	}
	...
	if( isset($_POST['userlog_open']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set Open to {$log_open}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置Open为{$log_open}。", CN_LANG);
	}
	if( isset($_POST['log_maxcount']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set MaxCount to {$log_maxcount}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置MaxCount为{$log_maxcount}。", CN_LANG);
	}
	if( isset($_POST['log_maxday']) )
	{
		...
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "set MaxDays to {$log_maxday}.");
		$log->VstorWebLog(LOG_INFOS, MOD_VIS, "设置MaxDays为{$log_maxday}。", CN_LANG);
	}
	
	// 保存配置
	$visprofile->Save();
	
	// 刷新页面（包括父页面，根据配置选择显示左侧菜单项）
	print "<script type=\"text/javascript\">";
	print "window.parent.location.href=\"vis.php?redirect=visconfig_target.php\";";
	print "</script>";
}
...
function vis_config_confirm(msg)
{
	if( confirm(msg) )
	{
		return true;
...
function check_manager_ip()
{
	...
	if(ipaddr == "")
	{
		document.getElementById("manager_ip_span").innerHTML = "<img src='./images/right_icon.png' style='visibility: hidden;'></img>";
		return true;
	}
...
<form id="vis_config_form" name="vis_config_form" action="visconfig_target.php" method="post">
...
<table width="60%" border="0" cellpadding="6" align="center">
  ...
  <tr>
    <td colspan="2">
	<input type="submit" name="vis_config_submit" onClick="return vis_config_confirm('<?php print $config_warn_str[$lang];?>');" value="<?php print $setup_str[$lang];?>">
